    ...
    if (this._httpProxy.currentUserAuthInfo && this._httpProxy.currentUserAuthInfo.access_token)
      if (
        ...
        if (req.method === 'GET' || req.method === 'HEAD') {
          req = req.clone({ setHeaders: { Authorization: `Bearer ${this._httpProxy.currentUserAuthInfo.access_token}` } , withCredentials: true });
        } else {
          let cookie = getCookie('XSRF-TOKEN');
          if (cookie) {
            req = req.clone({ setHeaders: { Authorization: `Bearer ${this._httpProxy.currentUserAuthInfo.access_token}`, 'X-XSRF-TOKEN': cookie }, withCredentials: true });
          } else {
